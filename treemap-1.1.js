/*
*	treemap plugin for jQuery (version 1.1pre 08/03/2010)
*	Copyright (c) 2007-2010 Renato Formato <rformato@gmail.com>
...
$.fn.treemap = function(w,h,options) {
	...
	return this.pushStack($.map(this,function(el){
		...
		if($.fn.treemap.caller!=treemap.layoutRow) {
			...
			if(!options.colorDiscreteVal) 
				options.colorDiscreteVal = {num:0};
			else {
				var num = 0;
				$.each(options.colorDiscreteVal,function(){
					num++;
				});
				options.colorDiscreteVal.num = num;
			}
		...
		if(options.target==el && $.nodeName(el,"table")) {
			var newObj = jQuery(el).children().insertBefore(el);
			$(el).remove();
			el = newObj.get();
		}
		return el; 
